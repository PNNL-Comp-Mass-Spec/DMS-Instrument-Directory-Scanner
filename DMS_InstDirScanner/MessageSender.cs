...
namespace DMS_InstDirScanner
{
    ...
    internal class MessageSender : EventNotifier, IDisposable
    {
        ...
        public void SendMessage(string message)
        {
            ...
            try
            {
                var textMessage = mSession.CreateTextMessage(message);
                textMessage.NMSTimeToLive = TimeSpan.FromMinutes(60);
                textMessage.NMSDeliveryMode = MsgDeliveryMode.NonPersistent;
                textMessage.Properties.SetString("ProcessorName", mProcessorName);
        ...
        public bool CreateConnection(int retryCount = 2, int timeoutSeconds = 15)
        {
            if (mHasConnection)
            {
                return true;
